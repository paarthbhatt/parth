...
/**
 * Declared at module scope on purpose. Defining it inside ProjectsSection would
 * make it a new component type on every parent render, remounting every card.
 * The staggered entrance is pure CSS so there's no per-card state or timer.
 */
function HackerProjectCard({
  project,
  index,
}: {
  project: (typeof securityProjectsData)[number]
  index: number
}) {
  return (
    <a
    ...
    >
      <div className="absolute inset-0 scanlines opacity-30 pointer-events-none" aria-hidden="true"></div>

      <div className="relative z-10">
        <div className="flex items-start justify-between mb-3">
          <div className="flex items-center gap-2">
            <span className="inline-block w-3 h-3 bg-emerald-500 rounded-full motion-safe:animate-pulse" aria-hidden="true"></span>
            <span className="text-xs font-mono text-emerald-500">{project.tag}</span>
          </div>
          <span className="text-xs font-mono text-emerald-400/80" aria-hidden="true">#{String(index + 1).padStart(2, '0')}</span>
        </div>
        ...
        <h3 className="text-base font-bold font-mono text-emerald-400 mb-2 group-hover:text-emerald-300 transition-colors">
          {project.title}
        </h3>
        ...
        <div className="mt-4 flex items-center justify-between text-xs font-mono" aria-hidden="true">
          <div className="flex items-center gap-2">
            <span className="text-emerald-500 motion-safe:animate-pulse">&gt;</span>
      ...
      <div className="container mx-auto max-w-7xl mt-6 space-y-8 relative z-10">
        ...
        <div className="scroll-reveal flex items-center justify-between mb-4 border-b border-emerald-500/20 pb-3">
          <div className="flex items-center gap-2">
            ...
            <h2 id="projects-heading" className="text-base sm:text-lg font-mono text-emerald-400 font-bold">
              <span aria-hidden="true">$ ./</span>projects<span aria-hidden="true"> --featured-security</span>
            </h2>
          </div>
          <span className="text-xs font-mono text-emerald-400/90">[{securityProjectsData.length} ACTIVE TARGETS]</span>
        </div>
        ...
        <div className="scroll-reveal delay-2 mt-8 sm:mt-12 bg-black border border-emerald-500/20 rounded-lg p-4 sm:p-6 shadow-[0_0_20px_rgba(16,185,129,0.15)]">
          <div className="flex items-center justify-between mb-4 border-b border-emerald-500/20 pb-2">
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-xs sm:text-sm font-mono text-emerald-400 font-semibold">Secondary repos and other builds</h3>
            </div>
            <span className="text-[10px] sm:text-xs font-mono text-emerald-400/80">hyperlinks active</span>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-2.5">
            {collapsedProjectsData.map((project, i) => (
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="group flex flex-col p-2.5 rounded border border-emerald-500/25 bg-emerald-500/5 hover:bg-emerald-500/10 hover:border-emerald-500/50 transition-all duration-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-400 focus-visible:ring-offset-2 focus-visible:ring-offset-black"
              >
                <div className="flex items-center justify-between mb-1">
                  <span className="text-[10px] font-mono text-emerald-400/90 group-hover:text-emerald-400 transition-colors">
                    [{project.tag}]
                  </span>
                  <span className="text-emerald-400/80 group-hover:translate-x-0.5 transition-transform text-xs">↗</span>
                </div>
